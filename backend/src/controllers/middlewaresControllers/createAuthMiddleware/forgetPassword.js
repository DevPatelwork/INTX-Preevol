...
const forgetPassword = async (req, res, { userModel }) => {
  ...
  const preevol_techinite_app_email = settings['preevol_techinite_app_email'];
  const preevol_techinite_base_url = settings['preevol_techinite_base_url'];

  const url = checkAndCorrectURL(preevol_techinite_base_url);
  ...
  await sendMail({
    ...
    name: user.name,
    link,
    subject: 'Reset your password | Preevol Techinite',
    preevol_techinite_app_email,
